-- Follow-up to 20260819120000_call_history.sql, kept as its own migration
-- rather than editing that file in place: if it was already applied,
-- rewriting it wouldn't rerun against a live Supabase project, and this
-- fix needs to reach databases in either state.
--
-- CREATE OR REPLACE only replaces a function when name AND argument types
-- match exactly. The prior migration added p_end_reason to
-- cancel_session_before_connect, which changed the signature — so on a
-- database where the original 2-arg version had already been created,
-- that migration left it in place as a second, ambiguous overload
-- alongside the new 3-arg one instead of truly replacing it. Drop both
-- possible prior signatures unconditionally before recreating.
DROP FUNCTION IF EXISTS public.cancel_session_before_connect(UUID, UUID);
DROP FUNCTION IF EXISTS public.cancel_session_before_connect(UUID, UUID, TEXT);

CREATE FUNCTION public.cancel_session_before_connect(
  p_session_id UUID,
  p_actor_id   UUID,
